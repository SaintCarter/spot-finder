...
### therefor:
{message: String} will never be used by the frontend.

{error: String} and {data: Object}
are the only response objects the frontend uses and needs to worry about handling from the backend. Otherwise it is a simple success or fail determined by the status number.
...
other than that, i believe, poisoning the jwt token, misconfigured endpoints, and denial of service are the systems main attack surfaces we need to handle.
...
## things we know we can do to prevent the attacks possible:
    ...
    - react
    6. LEAKED KEYS
    - take standard practice very seriously
